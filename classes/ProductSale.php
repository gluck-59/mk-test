...
class		ProductSale
{
	...
	*/	
	...
	/**
	* Добавлено мной
	* Нужно для вывода HOT у товаров
	*/	
	
	public static function isHot($id_product)
	{
		$result = Db::getInstance()->ExecuteS('
		SELECT `sale_nbr`
		FROM `presta_product_sale`
		WHERE id_product = '.$id_product.'
		');
		if ($result[0]['sale_nbr'] >= (Validate::isUnsignedInt(Configuration::get('PS_NB_HOT_PRODUCT')) ? Configuration::get('PS_NB_HOT_PRODUCT') : 30))
		return 1;
		else return 0;
	}
	...
	*/
	static public function getBestSales($id_lang, $pageNumber = 0, $nbProducts = 10, $orderBy=NULL, $orderWay=NULL, $days=NULL)
	{
		global $link, $cookie;

		if ($pageNumber < 0) $pageNumber = 0;
		if ($nbProducts < 1) $nbProducts = 50;
		if (empty($orderBy) || $orderBy == 'position') $orderBy = 'sales';
		if (empty($orderWay)) $orderWay = 'DESC';


		$result = Db::getInstance()->ExecuteS('
		SELECT IF(ps.`sale_nbr` > '.(Validate::isUnsignedInt(Configuration::get("PS_NB_HOT_PRODUCT")) ? Configuration::get("PS_NB_HOT_PRODUCT") : 30).',"1","0") as hot, p.*,
			...
			ps.`sale_nbr` AS sales, t.`rate`, pl.`meta_keywords`, pl.`meta_title`, pl.`meta_description`
		FROM `'._DB_PREFIX_.'product_sale` ps 
...
.($days ? 
'AND ps.`date_upd` between DATE_SUB(NOW(), INTERVAL '.$days.' DAY) AND NOW()' : 
'AND p.`id_product` IN (
	...
	*/
	static public function getBestSalesLight($id_lang, $pageNumber = 0, $nbProducts = 10)
	{
		...
		$result = Db::getInstance()->ExecuteS('
		SELECT p.id_product, pl.`link_rewrite`, pl.`name`, pl.`description_short`, i.`id_image`, il.`legend`, ps.`sale_nbr` AS sales, p.`ean13`, cl.`link_rewrite` AS category
		FROM `'._DB_PREFIX_.'product_sale` ps 
		LEFT JOIN `'._DB_PREFIX_.'product` p ON ps.`id_product` = p.`id_product`
		LEFT JOIN `'._DB_PREFIX_.'product_lang` pl ON (p.`id_product` = pl.`id_product` AND pl.`id_lang` = '.intval($id_lang).')
		LEFT JOIN `'._DB_PREFIX_.'image` i ON (i.`id_product` = p.`id_product` AND i.`cover` = 1)
	...
	static public function addProductSale($product_id, $qty = 4)
	{
		return Db::getInstance()->Execute('
