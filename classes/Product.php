...
// ограничение размеров поля	
	protected $fieldsSize = array('reference' => 128, 'supplier_reference' => 128, 'location' => 128, 'ean13' => 128);
...
//$fields['in_cart'] = $smarty->_tpl_vars['cart']->containsProduct($row['id_product'],null,null)['quantity'];
	...
	*/
	static public function getProducts($id_lang, $start, $limit, $orderBy, $orderWay, $id_category = false, $only_active = false, $quantity = false)
	{
	...
	/**
	* Добавлено мной
	* Нужно для вывода NEW у новых товаров в карточке товара
	*/	
	
	public static function isNewProduct($id_product)
	{
		$result = Db::getInstance()->ExecuteS('
			SELECT id_product FROM `'._DB_PREFIX_.'product` p
	...
	*/
	static public function getNewProducts($id_lang, $pageNumber = 0, $nbProducts = 10, $count = false, $orderBy = NULL, $orderWay = NULL)
	{
		...
		if (empty($orderBy) || $orderBy == 'position') $orderBy = 'date_upd';
		if (empty($orderWay)) $orderWay = 'DESC';
		if ($orderBy == 'id_product' OR $orderBy == 'price' OR $orderBy == 'date_upd')
			$orderByPrefix = 'p';
		elseif ($orderBy == 'name')
            $orderByPrefix = 'pl';
		if (!Validate::isOrderBy($orderBy) OR !Validate::isOrderWay($orderWay))
		...
		$result = Db::getInstance()->ExecuteS('
			SELECT  IF(ps.`sale_nbr` > '.(Validate::isUnsignedInt(Configuration::get("PS_NB_HOT_PRODUCT")) ? Configuration::get("PS_NB_HOT_PRODUCT") : 30).',"1","0") as hot, p.*, pl.`description`, pl.`description_short`, pl.`link_rewrite`, pl.`meta_description`, pl.`meta_keywords`, pl.`meta_title`, pl.`name`, p.`ean13`,
				i.`id_image`, il.`legend`, t.`rate`, m.`name` AS manufacturer_name, p.`active` as new, ps.`sale_nbr` as sales
			FROM `'._DB_PREFIX_.'product` p
	...
	*
	* вставил в несколько мест 	`on_sale` = 1
	* убрать если заглючит
	*/
	static public function getPricesDrop($id_lang, $pageNumber = 0, $nbProducts = 50, $count = false, $orderBy = NULL, $orderWay = NULL, $beginning = false, $ending = false)
	{
		global $link, $cookie;
		...
		$sql = '
		SELECT DISTINCT p.*, pl.`description`, pl.`description_short`, pl.`link_rewrite`, pl.`meta_description`, pl.`meta_keywords`, pl.`meta_title`, pl.`name`, p.`ean13`, i.`id_image`, il.`legend`, t.`rate`, (p.`reduction_price` + (p.`reduction_percent` * p.`price`)) AS myprice, m.`name` AS manufacturer_name
		FROM `'._DB_PREFIX_.'product` p
		LEFT JOIN `'._DB_PREFIX_.'product_lang` pl ON (p.`id_product` = pl.`id_product` AND pl.`id_lang` = '.intval($id_lang).')
		LEFT JOIN `'._DB_PREFIX_.'image` i ON (i.`id_product` = p.`id_product` AND i.`cover` = 1)
		LEFT JOIN `'._DB_PREFIX_.'image_lang` il ON (i.`id_image` = il.`id_image` AND il.`id_lang` = '.intval($id_lang).')
		...
		left join `presta_category_product` pcp on p.`id_product` = pcp.`id_product` 
		left join `presta_category` pc on pcp.`id_category` = pc.`id_category` 
		WHERE (`reduction_price` > 0 OR `reduction_percent` > 0 AND `on_sale` = 1 )
		'.((!$beginning AND !$ending) ?
			'AND (`reduction_from` = `reduction_to` OR (`reduction_from` <= \''.pSQL($currentDate).'\' AND `reduction_to` >= \''.pSQL($currentDate).'\'))'
		:
			($beginning ? 'AND `reduction_from` <= \''.pSQL($beginning).'\'' : '').($ending ? 'AND `reduction_to` >= \''.pSQL($ending).'\'' : '')).'
	...
	*/
	public function getAccessories($id_lang, $active = true)
	{
		...
		$result = Db::getInstance()->ExecuteS('
		SELECT p.*, pl.`description`, pl.`description_short`, pl.`link_rewrite`, pl.`meta_description`, pl.`meta_keywords`, pl.`meta_title`, pl.`name`, p.`ean13`,
		i.`id_image`, il.`legend`, t.`rate`, m.`name` as manufacturer_name, cl.`name` AS category_default, DATEDIFF(p.`date_add`, DATE_SUB(NOW(), INTERVAL '.(Validate::isUnsignedInt(Configuration::get('PS_NB_DAYS_NEW_PRODUCT')) ? Configuration::get('PS_NB_DAYS_NEW_PRODUCT') : 20).' DAY)) > 0 AS acc_new, ps.`sale_nbr` as acc_sales, IF(ps.`sale_nbr` > '.(Validate::isUnsignedInt(Configuration::get("PS_NB_HOT_PRODUCT")) ? Configuration::get("PS_NB_HOT_PRODUCT") : 30).',"1","0") as acc_hot
		FROM `'._DB_PREFIX_.'accessory`
		LEFT JOIN `'._DB_PREFIX_.'product` p ON p.`id_product` = `id_product_2`
		LEFT JOIN `'._DB_PREFIX_.'product_lang` pl ON (p.`id_product` = pl.`id_product` AND pl.`id_lang` = '.intval($id_lang).')
	...
	*/
	public function getAccessoriesRandom($id_lang, $qty)
	{
		...
		DATEDIFF(p.`date_upd`, DATE_SUB(NOW(), INTERVAL '.(Validate::isUnsignedInt(Configuration::get('PS_NB_DAYS_NEW_PRODUCT')) ? Configuration::get('PS_NB_DAYS_NEW_PRODUCT') : 20).' DAY)) > 0 AS acc_new, 
		ps.`sale_nbr` as acc_sales, IF(ps.`sale_nbr` > '.(Validate::isUnsignedInt(Configuration::get("PS_NB_HOT_PRODUCT")) ? Configuration::get("PS_NB_HOT_PRODUCT") : 30).',"1","0") as acc_hot
		FROM `'._DB_PREFIX_.'accessory`
		LEFT JOIN `'._DB_PREFIX_.'product` p ON p.`id_product` = `id_product_2`
		LEFT JOIN `'._DB_PREFIX_.'product_lang` pl ON (p.`id_product` = pl.`id_product` AND pl.`id_lang` = '.intval($id_lang).')
		LEFT JOIN `'._DB_PREFIX_.'category_lang` cl ON (p.`id_category_default` = cl.`id_category` AND cl.`id_lang` = '.intval($id_lang).')		
	...
	*/
	static public function searchByName($id_lang, $query)
	{
		...
		LEFT JOIN `'._DB_PREFIX_.'image` i ON (i.`id_product` = p.`id_product`) AND i.`cover` = 1
		LEFT JOIN `'._DB_PREFIX_.'image_lang` il ON (i.`id_image` = il.`id_image` AND il.`id_lang` = '.intval($id_lang).')
		WHERE pl.`name` LIKE \'%'.pSQL($query).'%\' OR p.`reference` LIKE \'%'.pSQL($query).'%\' OR p.`supplier_reference` LIKE \'%'.pSQL($query).'%\' OR p.`ean13` LIKE \'%'.pSQL($query).'%\' OR p.`id_product` LIKE \'%'.pSQL($query).'%\'
		GROUP BY `id_product`
...
*/
        ...
        $row['in_cart'] = $smarty->_tpl_vars['cart']->containsProduct($row['id_product'],null,null)['quantity'];
	...
	*/	
	...
	/*************** Random New Products *****************/
    /*
    * добавлено мной, если будет глючить, убить
    * ------ УЧИТЫВАЕТСЯ ДАТА РЕВИЗИИ ТОВАРА date_upd! -------
    * учитывается "считать товар новым Х дней" в админке
    *
    ...
    */
    static public function getNewProductsRandom($id_lang, $pageNumber = 0, $nbProducts = 10, $count = false, $orderBy = NULL, $orderWay = NULL)
    {
        ...
        if ($pageNumber < 0) $pageNumber = 0;
        if ($nbProducts < 1) $nbProducts = 10;
        if (empty($orderBy)) $orderBy = 'date_upd';
        if (empty($orderWay)) $orderWay = 'DESC';
        if ($orderBy == 'id_product' OR $orderBy == 'price' OR $orderBy == 'date_upd')
            $orderByPrefix = 'p';
        elseif ($orderBy == 'name')
        ...
        $result = Db::getInstance()->ExecuteS('
            SELECT IF(ps.`sale_nbr` > '.(Validate::isUnsignedInt(Configuration::get("PS_NB_HOT_PRODUCT")) ? Configuration::get("PS_NB_HOT_PRODUCT") : 30).',"1","0") as hot, p.*, pl.`description_short`, pl.`link_rewrite`, pl.`name`, i.`id_image`, il.`legend`, ps.`sale_nbr` as sales

            /*pl.`meta_description`, pl.`meta_keywords`, pl.`meta_title`, t.`rate`, m.`name` AS manufacturer_name, p.`ean13`, pl.`description`*/
...
/*            LEFT JOIN `'._DB_PREFIX_.'tax` t ON (t.`id_tax` = p.`id_tax`)
            LEFT JOIN `'._DB_PREFIX_.'manufacturer` m ON (m.`id_manufacturer` = p.`id_manufacturer`) */
            WHERE p.`active` = 1 AND p.`quantity` > 0 AND p.`id_category_default` > 1
            AND DATEDIFF(p.`date_upd`, DATE_SUB(NOW(), INTERVAL '.(Validate::isUnsignedInt(Configuration::get('PS_NB_DAYS_NEW_PRODUCT')) ? Configuration::get('PS_NB_DAYS_NEW_PRODUCT') : 20).' DAY)) > 0
            ORDER BY RAND() 
            LIMIT 0, '.Configuration::get('HOME_FEATURED_NBR'));
            //             LIMIT '.intval($pageNumber * $nbProducts).', '.intval($nbProducts) // оригинал
        if ($orderBy == 'price')
            Tools::orderbyPrice($result, $orderWay);
        if (!$result)
            return false;
