		...
		foreach ($words as $key => $word)
			if (!empty($word))
			{
				...
				$whereArray[] = ' p.id_product '.($word[0] == '-' ? 'NOT' : '').' IN (
					...
					AND sw.word LIKE '.($word[0] == '-' ? ' \''.pSQL(mb_substr($word, 1, PS_SEARCH_MAX_WORD_LENGTH, 'utf-8')).'%\'' : '\''.pSQL(mb_substr($word, 0, PS_SEARCH_MAX_WORD_LENGTH, 'utf-8')).'%\'').'
				) ';
				if ($word[0] != '-')
					$scoreArray[] = 'sw.word LIKE \''.pSQL(mb_substr($word, 0, PS_SEARCH_MAX_WORD_LENGTH, 'utf-8')).'%\'';
			}
			else
				unset($words[$key]);
		if (!sizeof($words))
