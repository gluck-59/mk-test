	...
	*/
	static public function convertPrice($price, $currency)
	{
		...
		if ($c_id != intval(Configuration::get('PS_CURRENCY_DEFAULT')))
			$price *= $c_rate;
		if ($c_id == 3)	return round($price,-2); // округляем рубли до -2 знака 
		return round($price);
	}
	...
	static public function getMetaTags($id_lang)
	{
		...
		if (!(isset($maintenance) AND (!isset($_SERVER['REMOTE_ADDR']) OR $_SERVER['REMOTE_ADDR'] != Configuration::get('PS_MAINTENANCE_IP'))))
		{
			...
			if ($id_product = Tools::getValue('id_product'))
			{
				...
				*/
				$row = Db::getInstance()->getRow('
				SELECT pl.`name`, pl.`meta_title`, pl.`meta_description`, pl.`meta_keywords`, pl.`description_short`, sup.`name` as supplier_name, man.`name` as manufacturer_name 
				FROM `'._DB_PREFIX_.'product_lang` pl
				left join `'._DB_PREFIX_.'product` p on p.id_product=pl.id_product  
				left join `'._DB_PREFIX_.'manufacturer` man on man.id_manufacturer=p.id_manufacturer 
				left join `'._DB_PREFIX_.'supplier` sup on sup.id_supplier=p.id_supplier 
				WHERE pl.id_lang = '.intval($id_lang).' AND pl.id_product = '.intval($id_product));
				if ($row)
				{
			...
			elseif ($id_cms = Tools::getValue('id_cms'))
			{
				...
				if ($row)
				{
//					$row['meta_title'] = Configuration::get('PS_SHOP_NAME').' - '.$row['meta_title']; // оригинал
...
//			$metaTags['meta_title'] = Configuration::get('PS_SHOP_NAME').' - '.$defaultValue; // оригинал
			$metaTags['meta_title'] = $defaultValue.' купить - '.Configuration::get('PS_SHOP_NAME');			
	...
	*/
	static public function getPath($id_category, $path = '', $linkOntheLastItem = false)
	{
		...
		$pipe = (Configuration::get('PS_NAVIGATION_PIPE') ? Configuration::get('PS_NAVIGATION_PIPE') : '&#10016');
	...
	*/
	...
	/*
	** Оригинальный link_rewrite, иногда глючил на имепрте CSV
	** 
	*/
...
*/
	...
	/*
	** Решение для "невозможно сохранить ... link_rewrite не заполнено для языка...
	** 
	*/
...
static public function link_rewrite($str, $utf8_decode = false)
    {
        ...
        if (preg_match('/[А-Яа-я]+/', $purified))
                ...
                $lat = array('a', 'b', 'v', 'g', 'd', 'e', 'jo', 'zh', 'z', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'r', 's', 't', 'u', 'f', 'h', 'c', 'ch', 'sh', 'sch', '', 'y', '', 'e', 'ju', 'ia', '_');
