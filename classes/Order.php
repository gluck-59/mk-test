...
class		Order extends ObjectModel
{
	...
	*/
	...
	{
		return Db::getInstance()->ExecuteS('
SELECT od.*, odl.`description`
FROM `presta_order_discount` od
join `presta_discount_lang` odl
where od.`id_discount` = odl.`id_discount`
AND `id_order` = '.intval($this->id));
	}
/*		SELECT *
...
*/
	...
	/*
	*
	* Получает список неоплаченных заказов за заданное время
	* Нет проверки на "отменено"!
	*
	*/
	static public function getOrdersRemindByDate($date_from, $date_to)
	{
		$result = Db::getInstance()->ExecuteS('
		SELECT `id_order`, DATE_FORMAT(`date_add`, "%Y%m%d") AS date_add
		FROM `presta_orders`
	...
	static public function getInvoice($id_invoice)
	{
		...
		/* ARPU общий, по юзерам */
	static public function getArpu()
	{
		return Db::getInstance()->ExecuteS('
			SELECT `id_customer`, sum(`total_paid_real`) / count(`id_customer`) as arpu, count(`id_customer`) as qty, year(`date_add`) as year, month(`date_add`) as month, DATE_FORMAT(`date_add`, "%b") as m
			FROM `presta_orders`
			where `valid` = 1
            and year(`date_add`) = year(NOW())			
		');
