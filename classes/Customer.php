...
class		Customer extends ObjectModel
{
	...
	protected $tables = array ('customer');
 	...
 	protected 	$fieldsRequired = array('lastname', 'passwd', 'firstname', 'email'); // оригинал
// 	protected 	$fieldsRequired = array('passwd', 'firstname', 'email'); 	
 	...
 	protected 	$fieldsValidate = array('secure_key' => 'isMd5', 'lastname' => 'isName', 'firstname' => 'isName', 'email' => 'isEmail', 'passwd' => 'isPasswd',
		 'id_gender' => 'isUnsignedId', 'birthday' => 'isBirthDate', 'newsletter' => 'isBool', 'optin' => 'isBool', 'active' => 'isBool'); // оригинал
/* 	protected 	$fieldsValidate = array('secure_key' => 'isMd5', 'firstname' => 'isName', 'email' => 'isEmail', 'passwd' => 'isPasswd',
	...
	public function getFields()
	{
		...
		$fields['lastname'] = pSQL(Tools::strtoupper($this->lastname)); // оригинал
//		($this->lastname != NULL ? $fields['lastname'] = pSQL(Tools::strtoupper($this->lastname)) : $fields['lastname'] = ' ');
	...
	/** 
	  * добавлено мной, убить если заглючит
	  * Возвращает customers list
	  * у которых есть wishlist
	  * @return array Customers
	  */
	static public function getCustomersWishlist()
	{
		return Db::getInstance()->ExecuteS('
		SELECT A.`id_customer`, `email`, `firstname`, `lastname`
    	FROM `'._DB_PREFIX_.'customer` A
   		INNER JOIN `'._DB_PREFIX_.'wishlist` B
	...
	/** 
	  * добавлено мной
	  * Возвращает customers list
	  * не забаненных и не удаленных
	  */
	static public function getCustomersNoBanned()
	{
		return Db::getInstance()->ExecuteS('
		SELECT `id_customer`, `email`, `firstname`, `lastname`
		FROM `'._DB_PREFIX_.'customer`
	...
	public static function getOptinemails()
	{
		return Db::getInstance()->ExecuteS('
		SELECT `email`, `firstname`, `lastname`, `optin`, `ip_registration_newsletter`, `newsletter_date_add`
		FROM `'._DB_PREFIX_.'customer`
		WHERE `optin` = 1
		AND `active` = 1');
	}
