...
abstract class AdminTab
{
			...
			if (!sizeof($this->_errors))
			{
				...
				else
				{
					if ($this->tabAccess['add'] === '1')
					{
						...
						$this->copyFromPost($object, $this->table);
						if (!$object->add())
							$this->_errors[] = Tools::displayError('an error occurred while creating object').' <b>'.$this->table.' ('.mysqli_error($GLOBALS["___mysqli_ston"]).')</b>';
	...
	public function displayConf()
	{
		...
		{
//			echo '<div class="conf confirm"><img src="../img/admin/ok.gif" /> '.$this->_conf[intval($conf)].'</div>'; // оригинал
echo '<script>toastr.success(\''.$this->_conf[intval($conf)].'\');</script>';				
		}
	}
