...
{if $total_price|replace:' ':'' >= $notax}
    ...
    <h2>Внимание, растаможка!</h2>
		...
		<p style="color: red"> 
			...
			С 1 января 2019 в России посылки стоимостью выше {$porog} &euro; облагаются ввозной пошлиной.<br> Стоимость доставки — считается!<br><br>
			Приблизительный  размер пошлины для этой посылки — <strong>{convertPrice price=$customs}</strong>. Пошлина оплачивается на месте при получении посылки.
		</p>
		<br>
		...
		<p>Если не согласен — убери какой-нибудь товар, чтобы общая стоимость посылки <strong> не превысила <nobr>{convertPrice price=$notax}</nobr></strong> с небольшим запасом на случай колебания курса евро.
		</p>
		<br><br>
		</div>
	</div>
