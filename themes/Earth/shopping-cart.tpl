...
{if $total_price|replace:' ':'' >= $notax}
    ...
    <h2>Внимание, растаможка!</h2>
		...
		<p style="color: red"> 
			...
			С 1 июля 2018 в России посылки стоимостью выше {$porog} &euro; облагаются ввозной пошлиной.<br> Стоимость доставки — считается!<br><br>
		...
		<p>Если нет — убери какой-нибудь товар, чтобы общая стоимость посылки <strong> не превысила <nobr>{convertPrice price=$notax}</nobr></strong> с небольшим запасом на случай колебания курса евро.
		</p>
		<br><br>
		</div>
	</div>
