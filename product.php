...
else
{
	$product = new Product(intval($_GET['id_product']), true, intval($cookie->id_lang));
		...
		foreach ($images AS $k => $image)
		{
			...
			// предзагрузим 
//			$img = (_PS_BASE_URL_.__PS_BASE_URI__.'img/p/'.$product->id.'-'.$image['id_image'].'-thickbox.jpg');
			$img = (_PS_IMG_URL_.__PS_BASE_URI__.'p/'.$product->id.'-'.$image['id_image'].'.jpg');
			$preload = array_merge(array($img), $preload);
		}
	...
	else
	{
		...
		$sales = intval(ProductSale::getNbrSales(intval($product->id)));
		$smarty->assign(array(
		'sales' => $sales
		));
        ...
        SELECT COUNT(`id_product`) as wishlists
        ...
        // свежачок
        $product_new = Product::isNewProduct($product->id);
        $product_new = $product_new[0]['id_product'];
        
        // HOT 
        $hot = ProductSale::isHot($product->id);
        
        // доставка в город 
        $SxGeo = new SxGeo('SxGeo/SxGeoCity.dat');
        $avg_delibery_days = new Carrier(55);
        $date = new DateTime();
        $interval = $avg_interval = $avg_delibery_days->avg_delibery_days;
        $ip = $_SERVER['HTTP_X_REAL_IP'];

        if (!$ip) 
        {
        	$date->add(new DateInterval('P'.round($interval).'D'));
        	$delivery_date = $date->format("d-m-Y H:i:s");
        }
        else
        {
        	...
        	$fast_suppliers = array('RIDERS DISCOUNT', 'www.louis.eu', 'RoMoto');
        	$roy_huy  = array('kakahealthcare', 'Meow-Auctshop', 'KK-moto');
        	
        	if ($geo['country']['iso'] != 'RU') $interval = $avg_interval - 7;
//            elseif ($geo['country']['iso'] = 'RU' && $product->reference == 'RoMoto') $interval = $avg_interval - 5; // для romoto // мешает остальным
        	elseif ($geo['country']['iso'] != 'RU' && $product->weight < 2) $interval = $avg_interval - 4;
        	elseif ($product->weight < 2 && !in_array($product->reference, $roy_huy) && $product->reference != 'www.louis.eu' ) $interval = $avg_interval + 15;
        	elseif (!in_array($product->reference, $fast_suppliers) && !in_array($product->reference, $roy_huy)) $interval = $avg_interval + 7;
        	elseif (in_array($product->reference, $roy_huy)) $interval = $avg_interval;

        	
        	$date->add(new DateInterval('P'.abs(intval($interval)).'D'));
        	$delivery_date = $date->format("d-m-Y");
        }

		$category_desc = Category::getCategoryDesc($category->id);
		...
		$smarty->assign(array(
			...
			'in_cart' => $smarty->_tpl_vars['cart']->containsProduct($product->id,null,null)
			));
...
//include_once 'product-accessories.php'; // аксессуары через ajax
