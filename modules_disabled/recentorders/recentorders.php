...
class recentOrders extends Module
{
	...
	function install()
	{
		if (!parent::install() OR !$this->registerHook('leftColumn') OR !Configuration::updateValue('RECENT_ORDER_TYPE', 'auto') OR !Configuration::updateValue('RECENT_ORDER_NUMBER', '10') OR !Configuration::updateValue('RECENT_ORDER_NUMBER_DISPLAY', '3'))
			return false;
		return true;
	}
	...
	{
		$this->_html = '<h3>'.$this->displayName.'</h3>';
	...
	function hookLeftColumn($params)
	{
		...
		if(Configuration::get('RECENT_ORDER_TYPE') == 'auto' || Configuration::get('RECENT_ORDER_TYPE') == 'mixed'){
			...
			$where = 'AND oh.`id_order_history` = (SELECT MAX(`id_order_history`) FROM `'._DB_PREFIX_.'order_history` moh WHERE moh.`id_order` = a.`id_order` AND a.`valid` = 1 GROUP BY moh.`id_order`)';
			...
			$sql = 'SELECT SQL_CALC_FOUND_ROWS
				...
				'.pSQL('DESC limit 0,'.Configuration::get('RECENT_ORDER_NUMBER').' ');

//printf(" (select: %.2f сек)<br>", (microtime(true) - $_SERVER["REQUEST_TIME_FLOAT"]));						
//echo (Configuration::get('RECENT_ORDER_NUMBER'));		
...
//echo '</p></pre>';
			foreach($list as $item)
			{
				...
				$result[] = array('product_name' => $products[0]['product_name'],
								  'product_link' => $product->getLink(),
...
//printf(" (foreach: %.2f сек)<br>", (microtime(true) - $_SERVER["REQUEST_TIME_FLOAT"]));						
