...
foreach ($sql AS $k=>$query){             
	...
	{
		$this->_html = '<h2>'.$this->displayName.' ('.count(Customer::getCustomersWishlist()).' юзерей)</h2>';
	...
	private function _displayFormSettings()
	{
		...
		$customers = Customer::getCustomersWishlist();
		if (!sizeof($customers))
			return;
...
<form><fieldset><legend>Статистика товаров</legend>';

// получаем список всех товаров во всех вишлистах
...
$this->_html .= '<div style=" height: 400px; overflow-y: scroll; overflow-x: hidden; "><table class="table">';
...
$qty = $products['qty'];
	...
	');
		...
		<td>' .$qty.'шт</td>
		<td><a href="http://motokofr.com/adminka/index.php?tab=AdminCatalog&id_product='.$id_product.'&updateproduct&token='.$token.'" target="_blank"><img src="http://motokofr.com/img/p/'.$id_product.'-'.$img['id_image'].'-small.jpg" />
		</td>
		<td>'.$name.'</a><br>';
	
			// получаем список юзерей с конкретным товаром
			...
			$email = $users[$i][email];
			$this->_html .=  '<a href="mailto:'.$email.'">'.$user.'</a>, ';
			}
//echo('<pre>');
//print_r($users);
}
...
//		global $cookie;
		...
		if ($params['cookie']->isLogged())
		{
			...
			if (empty($params['cookie']->id_wishlist) === true || WishList::exists($params['cookie']->id_wishlist, $params['cookie']->id_customer) === false)
			{
				if (!sizeof($wishlists))
				{
					// у юзера нет вишлистов, создаем
					$id_wishlist = false;
					$create_wishlist = 1;
//					echo('<!--jopa $id_wishlist = '.$id_wishlist.'-->');

				}
		
				else
				{
					// только что созданный (не нами) вишлист
