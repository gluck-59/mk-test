...
/* этот вариант рассылает только тем клиентам, которые что-то покупали
		$users = Db::getInstance()->ExecuteS('
		SELECT DISTINCT c.id_customer, firstname, lastname, email
		FROM '._DB_PREFIX_.'customer c
...
// добавление интервала (P1M = один месяц)
		...
		foreach ($users as $user)
		{
			...
			$voucher->name = 'birthday'.$user['id_customer'].date(y); //вот тут экспериментируем с названием купона
			$voucher->description[Configuration::get('PS_LANG_DEFAULT')] = $this->l('Your birthday present !');
//			$voucher->value = Configuration::get('BIRTHDAY_DISCOUNT_VALUE'); // оригинал -- сумма
			$voucher->value = 6;
			$voucher->quantity = 1;
...
//			$voucher->date_to = date('Y').(date('-m-d') - 1).date('-d');
//			$voucher->date_to = (date('Y') + 1).date('-m-d');			
			...
			if ($voucher) 
			 {
				$flag = $flag+1;			 
			 	if ($flag == 1) echo ("Юзеры проздравлены:\r\n");
				echo($voucher->name.' '.$user['firstname'].' '.$user['lastname'].' '.$user['email']."\r\n" );
			 }
			 else 
			 {
				echo ("Сегодня проздравлять некого.");
			 }
