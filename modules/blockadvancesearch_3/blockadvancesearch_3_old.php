...
elseif (preg_match('#^supplier#', $k)) {
				$supplier = true;
				$a [] = array ('typeid' => $k, 'name' => $this->l('У меня:') );
			}
		}
		...
		if (! $supplier) {
			$a [] = array ('typeid' => 'supplier', 'name' => $this->l('У меня:') );
		}
...
$filter_name = (isset($_GET ['filter']) && $_GET ['filter'] ? $_GET ['filter'] : false);
			...
			$nbProducts = $this->search(intval($cookie->id_lang), NULL, NULL, 'date_add', NULL, true, true, (count($ids_feature) ? $ids_feature : false), (count($ids_attribute) ? $ids_attribute : false), $gtPrice, $ltPrice, $id_category, $id_manufacturer, $id_supplier, $is_new, $is_promo,$on_stock,false,$filter_name);
			include (dirname(__FILE__) . '/../../pagination.php');
			include (dirname(__FILE__) . '/../../product-sort.php');
			
			$products = $this->search(intval($cookie->id_lang), intval($p), intval($n), $orderBy, $orderWay, false, true, (count($ids_feature) ? $ids_feature : false), (count($ids_attribute) ? $ids_attribute : false), $gtPrice, $ltPrice, $id_category, $id_manufacturer, $id_supplier, $is_new, $is_promo,$on_stock,Configuration::get('ADVC_COMB_IMG_'.$this->dupliq_id),$filter_name);
...
$rand_product = array(
"глушитель", "платформы", "дуги", "задние дуги", "багажник", "спинка", "зеркала", "крыло", "боковые крышки", "люстра", "накладка", "сиденье", "стекло", "рычаги", "прикуриватель", "решетка", "бугель", "пульт", "фильтр", "тахометр", "вынос", "ручки", "поворотник", "хомуты", "колонки", "часы"
);

// назначим tips для подсказок в зависимости от выбранной марки 
$last_supplier = explode("|",$_GET ['supplier']);
foreach ($last_supplier as $supp)
{
	$supp = $supp;
}
...
switch ($supp)
{
		case ($supp->id == 5) : $placeholder = trim($rand_product[array_rand($rand_product)]).' '.trim($tips[array_rand($tips)]).' '.array_rand(array(400=>0,600=>0,1100=>0,1300=>0,1800=>0)); break;
		case ($supp->id == 8) : $placeholder = trim($rand_product[array_rand($rand_product)]).' '.trim($tips[array_rand($tips)]).' '.array_rand(array(400=>0,800=>0,900=>0,1500=>0,1600=>0,2000=>0)); break;
		case ($supp->id == 9) : $placeholder = trim($rand_product[array_rand($rand_product)]).' '.trim($tips[array_rand($tips)]); break;
		case ($supp->id == 10) : $placeholder = trim($rand_product[array_rand($rand_product)]).' '.trim($tips[array_rand($tips)]).' '.array_rand(array(400=>0,650=>0,1100=>0,1300=>0,1600=>0)); break;
...
//echo "<pre>";print_r($order);
		...
		elseif (preg_match('#^supplier#', $k)) {
				...
				if (($stepMode && !$nextStepValue) || ($searchTypes [$k] == 3 || @count($advancedsearch_value ['supplier']) || $keep_empty_block))
					$advancedsearch_label ['supplier'] = array ('name' => $this->l('У меня:') );
/*			
echo '<!--';
...
elseif (preg_match('#^filter#', $k)) {
	  if (isset($_GET[$k]))
	  	   {
	  	   foreach (explode('|',$_GET[$k]) as $cfilter)
	  	   {
	  	   $aSelection []= array($cfilter,$cfilter,'По названию','filter','1');
		  	   //$aSelection []= array($_GET[$k],$_GET[$k],'По названию','filter','1');
	  	   }
		   
		   $nbProduct = $this->search(intval($cookie->id_lang), NULL, NULL, 'date_add', NULL, true, true, $ids_feature_value, $ids_attrubute_value, $gtPrice, $ltPrice, $id_category, $id_manufacturer, $id_supplier, $is_new, $is_promo,$on_stock,false,$_GET[$k]);
if (!$keep_empty_result && $nbProduct == 0 && ! $is_select)
							continue;
		   $total += $nbProduct;
		   $totalGroup += $nbProduct;
...
// добавление "универсальных" в вывод вызвает появление их в счетчике у "марок" ( OR p.`id_supplier` = 0 OR p.`id_supplier` = 12 )
				$where[] = '('.substr($critMultiple,0,-4).'  OR p.`id_supplier` = 0 OR p.`id_supplier` = 12)';
...
foreach (explode('|',$filter_name) as $cfilter) 
	{
		// обработаем ошибки ввода - используется "Инструменты - Псевднимы"
		$alias = new Alias(NULL, $cfilter);
		if ($alias->search) $cfilter = $alias->search;
		$where[] = 'pl.`name` LIKE \'%'.$cfilter.'%\' ';

//		$where[] = 'pl.`name` LIKE \'%'.$cfilter.'%\' and pl.`description` LIKE \'%'.$cfilter.'%\'';
//		$where[] = 'pl.`name` LIKE \'%'.$cfilter.'%\' or pl.`description` LIKE \'%'.$cfilter.'%\'';
//		$where[] = 'pl.`description` LIKE \'%'.$cfilter.'%\' ';		
	}
}
...
//echo "GET_TOTAL".$getTotal;
		...
		LEFT JOIN `'._DB_PREFIX_.'image_lang` il ON (i.`id_image` = il.`id_image` AND il.`id_lang` = '.intval($id_lang).')
		'.(count($where)?' WHERE '.implode(' AND ',$where):'');
		$sql .= ' GROUP BY p.`id_product` ORDER BY p.`id_supplier`, '.(isset($orderByPrefix) ? $orderByPrefix.'.' : '').'`'.pSQL($orderBy).'` '.pSQL($orderWay).'
