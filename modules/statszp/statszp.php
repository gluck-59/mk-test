...
class statszp extends ModuleGraph
{
    private $_html = '';
    private $_query = '';
    private $_query2 = '';
...
SELECT year(`date_add`) as year, MONTH(`date_add`) AS month, DATE_FORMAT(date_add, "%b") as m,  round(sum((`total_paid_real` - `total_products` + `total_shipping`) /1000)) as zp
FROM `presta_orders`
where `valid` = 1
...
');
	...
	public function getZarplataCurr()
	{
		return Db::getInstance()->ExecuteS('
		
SELECT month(`date_add`) as month, round(sum((`total_paid_real` - `total_products` + `total_shipping`)/1000), 1) as zp_curr
FROM `presta_orders`
where `valid` = 1
...
where `valid` = 1
					WHERE `date_add` BETWEEN ';
				$this->_query[1] = '
SELECT date_add, COUNT(DISTINCT `id_order`) as total
FROM `presta_orders`
where `valid` = 1
					WHERE `date_add` BETWEEN ';
