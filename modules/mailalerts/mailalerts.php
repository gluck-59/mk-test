    ...
    public function hookNewOrder($params)
    {
		...
		foreach ($params['cart']->getProducts() AS $key => $product)
		{
			...
			$itemsTable .=
				'<tr style="background-color:'.($key % 2 ? '#DDE2E6' : '#EBECEE').';">
					...
					<td style="padding:0.6em 0.4em; text-align:right;">'.Tools::displayPrice($unit_price, $currency, false, true).'</td>
					<td style="padding:0.6em 0.4em; text-align:center;">'.intval($product['quantity']).'</td>
					<td style="padding:0.6em 0.4em; text-align:right;">'.Tools::displayPrice(($price * $product['quantity']), $currency, false, true).'</td>
				</tr>';
		}
		foreach ($params['cart']->getDiscounts() AS $discount)
		{
			...
			'<tr style="background-color:#EBECEE;">
					...
					<td style="padding:0.6em 0.4em; text-align:right;">-'.Tools::displayPrice($discount['value_real'], $currency, false, true).'</td>
			</tr>';
		}
...
$cover = Product::getCover($id_product);
	...
	static public function getProductsAlerts($id_customer, $id_lang)
	{
		...
		for ($i = 0; $i < sizeof($products); ++$i)
		{
			...
			$products[$i]['sales'] = intval(ProductSale::getNbrSales(intval($obj->id)));
