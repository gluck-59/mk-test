...
class StatsBestCustomers extends ModuleGrid
{
	...
	public function getData()
	{
		...
		SELECT	c.`id_customer`, c.`lastname`, c.`firstname`, c.`email`,
			...
			count(DISTINCT oo.`id_order`) AS orders,
			COUNT(cop.`id_page`) AS totalPageViewed, (
				SELECT ROUND(SUM(IFNULL(o.`total_paid_real`, 0) / cu.conversion_rate))
		...
		left join `presta_orders` oo ON oo.`id_customer` = c.`id_customer`
