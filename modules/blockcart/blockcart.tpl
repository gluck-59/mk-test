...
{literal}<script>    $.ajaxSetup({ cache: false }); </script>{/literal}
...
<!--					<span class="quantity-formated"><span class="quantity">{$product.cart_quantity}</span>x</span> -->
					<a class="cart_block_product_name" href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category)}" title="{$product.name|escape:htmlall:'UTF-8'}">{t text=$product.name length='50' encode='true'}</a>
...
<!--								<span class="quantity-formated"><span class="quantity">{$customization.quantity}</span>x</span-->{if isset($customization.datas.$CUSTOMIZE_TEXTFIELD.0)}{t text=$customization.datas.$CUSTOMIZE_TEXTFIELD.0.value length='28' encode='true'}
			...
			{if $discounts|@count > 0}<table id="vouchers">
				...
				{foreach from=$discounts item=discount}
					<tr id="bloc_cart_voucher_{$discount.id_discount}">
						<td class="name" title="{$discount.description}">{$discount.name|escape:'htmlall':'UTF-8'}<br>{$discount.description|truncate:55:'...'|escape:'htmlall':'UTF-8'}</td>
	...
	<span id="ajax_tax" style="border-bottom: 1px dotted #F04B2D; margin-bottom:5px; cursor: help; font-style: italic; color: red" >
		...
		<p class="tooltip" tooltip="Они прилетают сами. Например на твой День Варенья ;)">Откуда берутся купоны? <img src="../../img/admin/help.png"></p>
	</span>
	...
	{/if}
			...
			<p id="cart-buttons">
    			{if $products|@count > 0}
				<a href="{$base_dir_ssl}order.php" class="ebutton green" title="{l s='Cart' mod='blockcart'}">{l s='Check out' mod='blockcart'}</a><br>
				{else}
                <a href="#" onclick="javascript:toastr.error('Твой кофр пока пуст.<br>Положи в него что-нибудь.');" class="ebutton green" title="{l s='Cart' mod='blockcart'}">{l s='Check out' mod='blockcart'}</a><br>
                {/if}
				{*<a href="{$base_dir_ssl}order.php?step=1" id="button_order_cart" class="ebutton green" title="{l s='Check out' mod='blockcart'}">{l s='Cart (button)' mod='blockcart'}</a>*}
			</p>
		</div>
		</div>
	</div>
