...
//			error: function() {alert('ERROR: unable to delete the product');}
			error: function() {alert('Удалить товар таким способом не получится. Удали его в средней колонке');}
...
/* fix *товар *товара *товаров */
	 var nnn = jsonData.nbTotalProducts % 100;
	if (nnn >= 5 && nnn <= 20) { $('.ajax_cart_product_txt_s:visible').hide(); $('.ajax_cart_product_txt_d:hidden').fadeIn('slow');
	} 
	else
	nnn = nnn % 10;
	if (nnn == 1) { $('.ajax_cart_product_txt_d:visible').hide(); $('.ajax_cart_product_txt:hidden').fadeIn('slow'); $('.ajax_cart_product_txt_s:visible').fadeOut('slow'); }
	if (nnn >= 2 && nnn <= 4) {  $('.ajax_cart_product_txt_d:visible').hide(); $('.ajax_cart_product_txt:visible').hide();		$('.ajax_cart_product_txt_s:hidden').show(); }
/* fix *товар *товара *товаров */
		}
		else
		{
			$('.ajax_cart_quantity:visible, .ajax_cart_product_txt_s:visible, .ajax_cart_product_txt_d:visible, .ajax_cart_product_txt:visible, .ajax_cart_total:visible').fadeOut('slow', function(){
				$('.ajax_cart_no_product:hidden').fadeIn('slow');
			});
		}
	}
