...
function copy2friendlyURL()
{
	str = getE('name_' + id_language).value;
	var Translit1 = new Translit();
	str = Translit1.UrlTranslit(str, 1);
	str = str.replace(/^[0-9]+\./, '');
	str = str.replace(/[ ]/g,'-');
	getE('link_rewrite_' + id_language).value = str2url(str, 'UTF-8');
}
...
// убить если заглючит
 function updateMeta()
{
	getE('meta_keywords_' + id_language).value = getE('tags_' + id_language).value,'UTF-8';
	getE('meta_description_' + id_language).value = getE('id="meta_description_' + id_language).value,'UTF-8';
}
