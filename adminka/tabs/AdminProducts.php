...
//			</ol></div>';
	...
	function displayFormInformations($obj, $currency, $languages, $defaultLanguage)
	{
		...
		<div class="tab-page" id="step1">
			...
			{
				echo '
			<a target="_blank" href="'.($link->getProductLink($this->getFieldValue($obj, 'id'), $this->getFieldValue($obj, 'link_rewrite', $defaultLanguage), Category::getLinkRewrite($this->getFieldValue($obj, 'id_category_default'), intval($cookie->id_lang)))).'"><img src="../img/admin/details.gif" alt="'.$this->l('View product in shop').'" title="'.$this->l('View product in shop').'" /> '.$this->l('View product in shop').'</a>';
		...
		foreach ($languages as $language)
			echo '			<div id="cname_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $defaultLanguage ? 'block' : 'none').'; float: left;">
								<input size="80" type="text" id="name_'.$language['id_lang'].'" name="name_'.$language['id_lang'].'"
								value="'.stripslashes(htmlspecialchars($this->getFieldValue($obj, 'name', $language['id_lang']))).'"'.((!$obj->id) ? ' onkeyup="copy2friendlyURL();"' : '').' onchange="updateCurrentText(); updateMeta();" />
...
&nbsp;&nbsp;&nbsp;<a href="?tab=AdminManufacturers&addmanufacturer&token='.Tools::getAdminToken('AdminManufacturers'.intval(Tab::getIdFromClassName('AdminManufacturers')).intval($cookie->id_employee)).'" onclick="return confirm(\''.$this->l('Are you sure you want to delete entered product information?', __CLASS__, true, false).'\');"><img src="../img/admin/add.gif" alt="'.$this->l('Create').'" title="'.$this->l('Create').'" /> <b>'.$this->l('Create').'</b></a>
		...
		echo '
					...
					<tr>
						<td class="col-left"><a target="_blank" href="https://www.ebay.com/sch/'.$this->getFieldValue($obj, 'ean13').'"><b>'.$this->l('Reference:').'</a></b></td>
						<td style="padding-bottom:5px;">
							<input size="55" type="text" name="reference" value="'.htmlentities($this->getFieldValue($obj, 'reference'), ENT_COMPAT, 'UTF-8').'" style="width: 209px; margin-right: 10px;" />
							товар<input type="text" name="ean13" value="'.$this->getFieldValue($obj, 'ean13').'" style="width: 280px; margin-left: 10px;" />
...
<option value="http://www.jafrum.com/s.nl?search=">Jafrum</option>
							</select-->
						</td>
					</tr>					
					<tr>
						...
						<td style="padding-bottom:5px;">
							...
							<span style=" margin-left: 150px; ">или&nbsp;</span><input size="55" type="text" name="location" value="'.$this->getFieldValue($obj, 'location').'" style="width: 270px; margin-left: 5px;" />
							<span class="hint" name="help_box">'.$this->l('Special characters allowed:').' .-_#\<span class="hint-pointer">&nbsp;</span></span>
...
<?php
				...
				echo '</td>
					...
					<tr>
						<td class="col-left">&nbsp;&nbsp;'.$this->l('available from').' </td>
						<td><input type="date" id="reduction_from" name="reduction_from" value="'.(($from = $this->getFieldValue($obj, 'reduction_from') AND $from != '0000-00-00' AND $from != '1942-01-01') ? $from : date('Y-m-d')).'" />
							'.$this->l('to').' <input type="date" id="reduction_to" name="reduction_to" value="'.(($to = $this->getFieldValue($obj, 'reduction_to') AND $to != '0000-00-00' AND $to != '1942-01-01') ? $to : date('Y-m-d')).'" />
...
$conversion_rate = new Currency(3);
$conversion_rate = (($conversion_rate->conversion_rate));


					
echo '<input hidden name="для price.js" id="conversion_rate" value="'.$conversion_rate.'">
					<tr>
						<td class="col-left"><b>'.$this->l('Final retail price:').'</b></td>
						<td style="padding-bottom:5px;">
							'.($currency->format == 1 ? $currency->sign.' ' : '').'<span id="finalPrice" style="font-weight: bold;"></span> <b>— <span id="finalPriceRur"></span> руб</b>'.($currency->format == 2 ? ' '.$currency->sign : '').'
						</td>
					</tr>
					<tr><td colspan="2" style="padding-bottom:5px;"><hr style="width:730px;"></td></tr>
					<tr>
		...
		foreach ($languages as $language)
			echo '
								...
								value="'.stripslashes(htmlspecialchars($this->getFieldValue($obj, 'meta_description', $language['id_lang']))).'"'.((!$obj->id) ? ' onkeyup="copy2friendlyURL();"' : '').' onchange="updateCurrentText();" />
											<span class="hint" name="help_box">'.$this->l('Forbidden characters:').' <>;=#{}<span class="hint-pointer">&nbsp;</span></span>
										</div>';
		...
		echo '
								...
								<tr>
									...
									<td>';
		foreach ($languages as $language)
			echo '<div id="cmeta_keywords_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $defaultLanguage ? 'block' : 'none').'; float: left;">
		...
		foreach ($languages as $language)
			echo '
							<div class="noscroll" id="cdesc_short_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $defaultLanguage ? 'block' : 'none').';float: left;">
		...
		foreach ($languages as $language)
			echo '
							<div class="noscroll" id="cdesc_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $defaultLanguage ? 'block' : 'none').';float: left;">
		...
		echo '
				...
				foreach ($languages as $language)
				{
					echo '<div id="ctags_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $defaultLanguage ? 'block' : 'none').'; float: left;">
							...
							value="'.htmlentities(Tools::getValue('tags_'.$language['id_lang'], $obj->getTags($language['id_lang'], true)), ENT_COMPAT, 'UTF-8').'" onkeyup="updateMeta();" />
...
if ($images) echo '<img src="../img/p/'.$obj->id.'-'.$images[0]['id_image'].'-large.jpg" style="  position: absolute;
  top: 0;
  z-index: 2;
  left: 766px;">';
		...
		<div class="tab-page" id="step2">
				...
				<table cellpadding="5" style="width:100%">
					<tr>
						...
						<td style="padding-bottom:5px;">
							<input multiple style="height:70px; width:97%; border-radius:10px; background-color:#AFA" type="file" id="image_product" name="image_product" ondrop="submitAddproduct" />
			...
			<tr>
					...
					<img src="../img/p/'.$obj->id.'-'.$image['id_image'].'-home.jpg"
		...
		if ($obj->id)
		  ...
		  <tr>
			  ...
			  <td style="padding-bottom:5px;">
				...
				'.$this->l('EAN13:').'<input size="55" type="text" id="attribute_ean13" name="attribute_ean13" value="" style="width: 110px; margin-left: 10px;" />
				<span class="hint" name="help_box">'.$this->l('Special characters allowed:').' .-_#<span class="hint-pointer">&nbsp;</span></span>
			  </td>
		  </tr>
		  <tr>
...
$(document).ready(function () {
	$('#cdesc_3').hover(
      function () 
      {
        $('body#adminka').css('overflow', 'hidden');
      }, 
    
      function () 
      {
        $('body#adminka').css('overflow', 'scroll');
      }
    );
