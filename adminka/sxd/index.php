...
class Sypex_Dumper {
    ...
    function connect($host = null, $port = null, $user = null, $pass = null){
        ...
        if(($GLOBALS["___mysqli_ston"] = mysqli_connect($this->CFG['my_host'] . ($this->CFG['my_host']{0} != ':' ? ":{$this->CFG['my_port']}" : ''),   $this->CFG['my_user'],  $this->CFG['my_pass']))) {
            if(V_PHP > 50202) ((bool)mysqli_set_charset($GLOBALS["___mysqli_ston"], "utf8")) or sxd_my_error();
            else mysqli_query($GLOBALS["___mysqli_ston"], 'SET NAMES utf8') or sxd_my_error();
            define('V_MYSQL', sxd_ver2int(((is_null($___mysqli_res = mysqli_get_server_info($GLOBALS["___mysqli_ston"]))) ? false : $___mysqli_res)));
        }
        else {
            define('V_MYSQL', 0);
            $this->error = "sxd.actions.tab_connects();alert(" . sxd_esc(mysqli_error($GLOBALS["___mysqli_ston"])) . ");";
        }
        $this->try = false;
    ...
    function deleteDB($name){
        $r = mysqli_query($GLOBALS["___mysqli_ston"], 'DROP DATABASE `' . sxd_esc($name, false) . '`') or sxd_my_error();
        if($r){
            echo "sxd.clearOpt('db');sxd.addOpt(" . sxd_php2json(array('db' => $this->getDBList())) . ");sxd.combos.services_db.select(0,'-');";
        }
        else
            echo "alert(" . sxd_esc(mysqli_error($GLOBALS["___mysqli_ston"])) . ");";
    }
    function cfg2js($cfg){
        foreach($cfg AS $k => $v){
    ...
    function addDb($req){
        $r = mysqli_query($GLOBALS["___mysqli_ston"], 'CREATE DATABASE `' . sxd_esc($req['name'], false) . '`' . (V_MYSQL > 40100 ? "CHARACTER SET {$req['charset']} COLLATE {$req['collate']}" : ''));
        if($r)
    ...
    function runServices($job) {
        ...
        if(isset($serv[$job['type']])) {
            mysqli_select_db($GLOBALS["___mysqli_ston"], $job['db']);
            $filter = $object = array();
            $this->createFilters($job['obj'], $filter, $object);
            $r = mysqli_query($GLOBALS["___mysqli_ston"], 'SHOW TABLE STATUS') or sxd_my_error();
            if (!$r) return;
            $tables = array();
            ...
            $r = mysqli_query($GLOBALS["___mysqli_ston"], $sql) or sxd_my_error();
            if (!$r) return;
            $res = array();
    ...
    function runRestoreJob($continue = false){
        ...
        mysqli_select_db($GLOBALS["___mysqli_ston"], $this->JOB['db']);
        ...
        if($this->JOB['strategy'] > 0){
            $r = mysqli_query($GLOBALS["___mysqli_ston"], "SHOW TABLES") or sxd_my_error();
            while($item = mysqli_fetch_row($r)){
        ...
        //$enable_index = array();
        if(V_MYSQL > 40014) {
            mysqli_query($GLOBALS["___mysqli_ston"], "SET UNIQUE_CHECKS=0");
            mysqli_query($GLOBALS["___mysqli_ston"], "SET FOREIGN_KEY_CHECKS=0");
            if(V_MYSQL > 40101) mysqli_query($GLOBALS["___mysqli_ston"], "SET SQL_MODE='NO_AUTO_VALUE_ON_ZERO'");
            if(V_MYSQL > 40111) mysqli_query($GLOBALS["___mysqli_ston"], "SET SQL_NOTES=0");
        }
        $log_sql = false;
        $fields = '';
        $time_old = time();
            ...
            switch($q{0}){
                ...
                case 'I':
                    if (preg_match('/^INSERT( INTO `(.+?)`) VALUES/', $q, $m)) {
                        ...
                        mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE `{$tab}` DISABLE KEYS") or sxd_my_error();
...
//                        $this->setNames($this->JOB['correct'] == 1 && !empty($this->JOB['charset']) ? $this->JOB['charset'] : empty($m[3]) ? '' : $m[3]);
                        ...
                        if($m[1] == 'TC') {
                            ...
                            if($tc && ($this->JOB['strategy'] == 0 || isset($tab_exists[$m[2]]))) {
                                mysqli_query($GLOBALS["___mysqli_ston"], "{$tc} `{$m[2]}`") or sxd_my_error();
                            }
                        }
						elseif($m[1] == 'TD'){
                            $ei = 1;
                            ...
                            $this->rtl[7] = 0;
                            $this->rtl[8] = 0;
                            mysqli_query($GLOBALS["___mysqli_ston"], "DROP {$types[$m[1]]} IF EXISTS `{$m[2]}`") or sxd_my_error();
            ...
            if($ex) {
                ...
                $this->rtl[1] = time();
                fwrite($this->fh_rtl, implode("\t", $this->rtl));
                if(mysqli_query($GLOBALS["___mysqli_ston"], $q)) {
                    if($insert) {
                        $c = 1;
                    }
                }
                ...
                if($c){
                    $i = mysqli_affected_rows($GLOBALS["___mysqli_ston"]);
                    $this->rtl[3] = ftell($this->fh_tmp) - $seek;
                    $this->rtl[7] += $i;
                    $this->rtl[10] += $i;
                    fseek($this->fh_rtl, 0);
        ...
        foreach($this->JOB['todo']['TA'] AS $tab){
            ...
            mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE `{$tab[1]}` ENABLE KEYS") or sxd_my_error();
            $this->rtl[1] = time();
            $this->rtl[5] = $tab[1];
            fseek($this->fh_rtl, 0);
            fwrite($this->fh_rtl, implode("\t", $this->rtl));
    ...
    function addBackupJob($job) {
        ...
        foreach($queries AS $query){
            ...
            if($t == 'TA' && (!empty($object['TC']) || !empty($filter['TC']))) {}
			elseif(empty($object[$t]) && empty($filter[$t])) continue;
            $r = mysqli_query($GLOBALS["___mysqli_ston"], 'SHOW ' . $query[0]) or sxd_my_error();
            if (!$r) continue;
            $todo[$t] = array();
            $header[$t] = array();
        ...
        if (V_MYSQL > 50014 && (!empty($object['VI']) || !empty($filter['VI']))) {
            ...
            $r = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT table_name, view_definition /*!50121 , collation_connection */ FROM INFORMATION_SCHEMA.VIEWS WHERE TABLE_SCHEMA = '{$this->JOB['db']}'") or sxd_my_error();
    ...
    function runBackupJob($continue = false){
        ...
        mysqli_query($GLOBALS["___mysqli_ston"], "SET SQL_QUOTE_SHOW_CREATE = 1");
        $types = array('VI' => 'View', 'PR' => 'Procedure', 'FU' => 'Function', 'TR' => 'Trigger', 'EV' => 'Event');
        $fcache = '';
        $writes = 0;

        if(V_MYSQL > 40101) mysqli_query($GLOBALS["___mysqli_ston"], "SET SESSION character_set_results = '" . ($this->JOB['charset'] ? $this->JOB['charset'] : 'binary') ."'") or sxd_my_error();
        $time_old = time();
        $exit_time = $time_old + $this->CFG['time_web'] - 1;
                ...
                switch($n[0]){
                    ...
                    case 'TA':
                        ...
                        if ($n[0] == 'TC' || $this->rtl[7] == 0){
                            ...
                            $r = mysqli_query($GLOBALS["___mysqli_ston"], "SHOW CREATE TABLE `{$n[1]}`") or sxd_my_error();
                        ...
                        $notNum = array();
                        $r = mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM `{$n[1]}`") or sxd_my_error();
                        $fields = 0;
                        ...
                        $z = 0;
                        // Достаем данные
                        $r = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT {$no_cache}* FROM `{$n[1]}`{$from}", MYSQLI_USE_RESULT);
                        while($row = mysqli_fetch_row($r)) {
                            ...
                            for($k = 0; $k < $fields; $k++){
								...
								elseif($notNum[$k]) {$row[$k] =  '\'' . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $row[$k]) . '\'';} // TODO: Потестить скорость эскэйпинга строк
                            }
                            $fcache .= '(' . implode(',', $row) . "),\n";
                            $this->rtl[7]++;
                            $this->rtl[10]++;
                    ...
                    default:
                        if(V_MYSQL < 50121 && $n[0] == 'TR'){
                            ...
                            $r = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM `INFORMATION_SCHEMA`.`TRIGGERS` WHERE `TRIGGER_SCHEMA` = '{$this->JOB['db']}' AND `TRIGGER_NAME` = '{$n[1]}'") or sxd_my_error();
                            $item = mysqli_fetch_assoc($r);
                            ...
                            $r = mysqli_query($GLOBALS["___mysqli_ston"], "SHOW CREATE {$types[$n[0]]} `{$n[1]}`") or sxd_my_error();
                            $item = mysqli_fetch_assoc($r);
    ...
    function setNames($collation){
        ...
        if($this->rtl[6] != $collation) {
            mysqli_query($GLOBALS["___mysqli_ston"], 'SET NAMES \'' . preg_replace('/^(\w+?)_/', '\\1\' COLLATE \'\\1_', $collation) . '\'') or sxd_my_error();
    ...
    function getDBList(){
        ...
        if ($this->CFG['my_db']) {
            ...
            $result = mysqli_query($GLOBALS["___mysqli_ston"], "SHOW DATABASES") or sxd_my_error();
        ...
        if(V_MYSQL < 50000){
            foreach($items AS $item){
                $tables = mysqli_query($GLOBALS["___mysqli_ston"], "SHOW TABLES FROM `{$item}`") or sxd_my_error();
                if ($tables) {
            ...
            $result = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT `table_schema`, COUNT(*) FROM `information_schema`.`tables` {$where} GROUP BY `table_schema`") or sxd_my_error();
            while($item = mysqli_fetch_row($result)){
                if($item[0] == 'information_schema' || $item[0] == 'mysql' || $item[0] == 'performance_schema') continue;
    ...
    function getCharsetList(){
        ...
        if(V_MYSQL > 40101) {
            ...
            $r = mysqli_query($GLOBALS["___mysqli_ston"], "SHOW CHARACTER SET") or sxd_my_error();
            if ($r) {
                while($item = mysqli_fetch_assoc($r)){
    ...
    function getCollationList(){
        ...
        if(V_MYSQL > 40101) {
            ...
            $r = mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLLATION") or sxd_my_error();
            if ($r) {
                while($item = mysqli_fetch_assoc($r)){
    ...
    function getObjects($tree, $db_name){
        ...
        // Достаем таблицы
        $r = mysqli_query($GLOBALS["___mysqli_ston"], 'SHOW TABLE STATUS');
        ...
        if($r){
            ...
            if (V_MYSQL > 50014 && $tree != 'services') {
                ...
                for($i = 0, $l = count($shows); $i < $l; $i++){
                    $r = mysqli_query($GLOBALS["___mysqli_ston"], 'SHOW ' . $shows[$i]);
...
function sxd_my_error(){
    trigger_error(mysqli_error($GLOBALS["___mysqli_ston"]), E_USER_ERROR);
}
