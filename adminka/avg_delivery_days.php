...
//echo ($track['courier_slug'].' '.$track['tracking_number'].'<br>');
    ...
    if ($avg_delibery_days_ems && $avg_delibery_days_ems != 0 && $avg_delibery_days_ems < 300)
        $result_ems = mysqli_query($GLOBALS["___mysqli_ston"], 'update presta_carrier set `avg_delibery_days` = '.intval($avg_delibery_days_ems).' where `id_carrier` = 74');
    ...
    if ($avg_delibery_days_post && $avg_delibery_days_post != 0 && $avg_delibery_days_post < 300)
        $result_post = mysqli_query($GLOBALS["___mysqli_ston"], 'update presta_carrier set `avg_delibery_days` = '.intval($avg_delibery_days_post).' where `id_carrier` = 79');
