...
//print_r($track_id);
	...
	foreach ($shipping_numbers as $shipping_number) 
	{
		...
		$track = simplexml_load_file('http://gamma.gdeposylka.ru/ws/x1/track.status/xml/?apikey=180.ba2ef1012d&id='.$shipping_number);
// https://code.google.com/p/gdeposylka-api/wiki/API_X1
		if ($track===false)
			{
				echo('Gdeposylka в дауне<br>');
				//exit;
			}
...
//		if ($track->tracks->track->code == "403") // если трек неизвестный, то добавляем
		if ($track->tracks->track->code > "0") // если трек неизвестный, то добавляем		
			{
				file_get_contents('http://gamma.gdeposylka.ru/ws/x1/track.add/xml/?apikey=180.ba2ef1012d&id='.$shipping_number.'&country='.$country_id.'&descr='.sprintf('%04d',$order_id));
