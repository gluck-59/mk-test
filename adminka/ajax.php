...
if (isset($_GET['ajaxProductAccessories']))
{
	...
	foreach ($products AS $accessory)
		$jsonArray[] = '{value: \''.intval($accessory['id_product']).'-'.addslashes($accessory['name']).'\', text:\''.intval($accessory['id_product']).' — '.addslashes($accessory['name']).' — '.addslashes($accessory['price']).'р — '.addslashes($accessory['reference']).'\'}';
	die('['.implode(',', $jsonArray).']');
}
...
//		$jsonArray[] = '{value: \''.intval($packItem['id_product']).'-'.addslashes($packItem['name']).'\', text:\''.intval($packItem['id_product']).' - '.addslashes($packItem['name']).'\'}';
		$jsonArray[] = '{value: \''.intval($packItem['id_product']).'-'.addslashes($packItem['name']).'\', text:\''.intval($packItem['id_product']).' - '.addslashes($packItem['name']).' — '.addslashes($packItem['reference']).'\'}';
