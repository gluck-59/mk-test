...
function displayAddress()
{
	...
	// предзагрузим 
	$preload = array(_PS_BASE_URL_.__PS_BASE_URI__.'order.php?step=2');
	Tools::safePostVars();
...
echo '</pre-->';*/
	...
	// предзагрузим 
	$preload = array(_PS_BASE_URL_.__PS_BASE_URI__.'order.php?step=3');
	Tools::safePostVars();
	$css_files = array(__PS_BASE_URI__.'css/thickbox.css' => 'all');
//	$js_files = array(__PS_BASE_URI__.'js/jquery/thickbox-modified.js'); // не используется и тормозит
	include_once(dirname(__FILE__).'/header.php');
...
function displayPayment()
{
	...
	// предзагрузим 
	//$preload = array(0 => _PS_BASE_URL_.__PS_BASE_URI__.'order.php?step=modules/paypalapi/payment/submit.php', 1 => 'https://merchant.webmoney.ru/lmi/SignedLoginFormNewWC.asp?at=authtype_16');
	Tools::safePostVars();
	include_once(dirname(__FILE__).'/header.php');
...
// получим содержимое текущего вишлиста юзера и отдадим его в shopping-cart.tpl
include_once(dirname(__FILE__).'/modules/blockwishlist/WishList.php');
$wishlist = Wishlist::getProductByIdCustomer(intval($cookie->id_wishlist), intval($cookie->id_customer), intval($cookie->id_lang));
...
//получаем беспошлинную сумму и порог обложения из config.inc.php
//передаем в средний фрейм "содержимое кофра"
		'notax' => $GLOBALS['notax'],	
		'porog'	=> $GLOBALS['porog'],
		'wishlist' => $wishlist
//		'lastProductAdded' => $cart->getLastProduct()
	...
	// предзагрузим 
	$preload = array(_PS_BASE_URL_.__PS_BASE_URI__.'order.php?step=1');
	Tools::safePostVars();
	include_once(dirname(__FILE__).'/header.php');
	$smarty->display(_PS_THEME_DIR_.'shopping-cart.tpl');
